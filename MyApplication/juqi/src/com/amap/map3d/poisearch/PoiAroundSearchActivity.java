...
public class PoiAroundSearchActivity extends Activity implements
		OnClickListener, OnMapClickListener, OnInfoWindowClickListener,
		InfoWindowAdapter, OnMarkerClickListener, OnPoiSearchListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mIntent = getIntent();
		lp = new LatLonPoint(mIntent.getDoubleExtra("lat", 0),
				mIntent.getDoubleExtra("lon", 0));
	 ...
	 * 初始化AMap对象
	 */
	private void init(LatLonPoint lp) {
		if (mAMap == null) {
	...
	private void setup() {
		...
		mPoiDetail.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				bundle.putString("result", addressTitle);
				bundle.putString("detailPlace", addressDetail);
				bundle.putDouble("longtitude", lon);
				bundle.putDouble("latitude", lat);
				bundle.putString("district", district);
				resultIntent.putExtras(bundle);
				Config.t_lat = lat;
				Config.t_lon = lon;
				PoiAroundSearchActivity.this.setResult(RESULT_OK, resultIntent);
	...
	protected void doSearchQuery() {
		...
		if (lp != null) {
			...
			poiSearch.setBound(new SearchBound(lp, 1000000, true));//
				...
				if (result.getQuery().equals(query)) {// 是否是同一条
					...
					if (poiItems != null && poiItems.size() > 0) {
						...
						mAMap.addCircle(new CircleOptions()
								.center(new LatLng(lp.getLatitude(), lp
	...
	private void setPoiItemDisplayContent(final PoiItem mCurrentPoi) {
		...
		addressTitle = mCurrentPoi.getTitle();
		addressDetail = mCurrentPoi.getSnippet();
		district = mCurrentPoi.getAdName();
