...
import cn.com.sfn.juqi.util.AMapUtil;
...
public class RouteActivity extends Activity implements OnMapClickListener,
	...
	private double sPointlat, sPointlon, ePointlat, ePointlon;
	...
	protected void onCreate(Bundle bundle) {
		...
		mIntent = getIntent();
		sPointlat = mIntent.getDoubleExtra("slat", Config.lat);
		sPointlon = mIntent.getDoubleExtra("slon", Config.lon);
		ePointlat = mIntent.getDoubleExtra("elat", 39.984947);
		ePointlon = mIntent.getDoubleExtra("elon", 116.494689);

		mStartPoint = new LatLonPoint(sPointlat, sPointlon);// 起点，
		mEndPoint = new LatLonPoint(ePointlat, ePointlon);// 终点，
	...
	private void setfromandtoMarker(LatLonPoint mStart, LatLonPoint mEnd) {
		aMap.addMarker(new MarkerOptions().position(
				AMapUtil.convertToLatLng(mStart)).icon(
				BitmapDescriptorFactory.fromResource(R.drawable.start)));
		aMap.addMarker(new MarkerOptions().position(
				AMapUtil.convertToLatLng(mEnd)).icon(
